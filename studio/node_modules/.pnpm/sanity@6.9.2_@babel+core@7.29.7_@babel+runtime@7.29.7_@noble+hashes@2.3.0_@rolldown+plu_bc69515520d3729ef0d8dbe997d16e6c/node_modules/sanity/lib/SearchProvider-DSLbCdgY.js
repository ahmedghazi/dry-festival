import { t as ErrorBoundary$1 } from "./ErrorBoundary-CNVRCu1C.js";
import { n as Tooltip, t as Button$1 } from "./Button-CGZ3ETwD.js";
import { n as MenuButton$1, t as MenuItem$1 } from "./MenuItem-D0rtIQmn.js";
import { t as Popover } from "./Popover-DRCM1MS-.js";
import { C as isNonNullable, l as getPublishedId$1, x as removeDupes } from "./draftUtils-D3HJGQaW.js";
import { c as useWorkspace, i as useClient, n as DEFAULT_STUDIO_CLIENT_OPTIONS, o as useSource, r as versionedClient } from "./studioClient-WYiCwgfZ.js";
import { $t as variantApiVersion, Zt as fieldNeedsEscape, a as useGrantsStore, dn as isRecord, in as isReleasePerspective, l as useProjectStore, pn as useSchema, r as useDocumentPreviewStore, rn as RELEASES_STUDIO_CLIENT_OPTIONS, s as useKeyValueStore, u as useRenderingContextStore } from "./datastores-D4iZNSXV.js";
import { t as useTranslation } from "./useTranslation-Cqs6Itwv.js";
import { y as EMPTY_ARRAY } from "./DateInputs-enUB1fG8.js";
import { t as TextWithTone } from "./TextWithTone-DDdV-YQo.js";
import { n as useCurrentUser } from "./LoadingBlock-Cy2Pckab.js";
import { t as CommandList } from "./CommandList-CPEbgHpU.js";
import { t as Translate } from "./Translate-0HIWr1eF.js";
import { i as getPreviewStateObservable, n as DocumentPreviewPresence, r as useDocumentPresence } from "./useDocumentPreviewValues-B4jUnACZ.js";
import { C as useDocumentVersions, V as SanityDefaultPreview, ht as useActiveReleases, k as useValuePreview } from "./useBundleDocuments-CiRQnzQh.js";
import { r as truncateString } from "./ReleaseTitle-CkEMIfmx.js";
import { n as DocumentStatus, t as DocumentStatusIndicator } from "./DocumentStatusIndicator-BP3N2GwG.js";
import { C as getDocumentVersionInfoFromVersions } from "./useScheduledDraftMenuActions-DqsFlU0p.js";
import { Fragment, isValidElement, memo, useContext, useDeferredValue, useEffect, useId, useReducer, useRef, useState } from "react";
import { EMPTY, Subject, catchError, concat, iif, map, merge, mergeMap, of, startWith, timer } from "rxjs";
import { c } from "react/compiler-runtime";
import { Fragment as Fragment$1, jsx, jsxs } from "react/jsx-runtime";
import { Badge, Box, Button, Card, Container, Flex, Inline, Portal, Stack, Text, TextInput, rem, studioTheme, useClickOutsideEvent, useGlobalKeyDown, useLayer, useMediaIndex } from "@sanity/ui";
import { AppIdCacheContext, PreviewCardContext, SearchContext } from "sanity/_singletons";
...
import { catchError as catchError$1, debounce as debounce$1, distinctUntilChanged as distinctUntilChanged$1, filter as filter$1, map as map$1, scan as scan$1, shareReplay as shareReplay$1, startWith as startWith$1, switchMap as switchMap$1, tap as tap$1 } from "rxjs/operators";
...
import { isIndexSegment, isIndexTuple, isKeySegment, isObjectSchemaType, isReferenceSchemaType } from "@sanity/types";
...
/** @internal */
const StyledCard = /* @__PURE__ */ styled(Card).withConfig({
	displayName: "StyledCard",
	componentId: "sc-mkoxdv-0"
})(() => css`&[data-selected],&[data-pressed],&:active{[data-ui='TextWithTone']{color:inherit;}}`);
/** @internal */
function usePreviewCard() {
	let context = useContext(PreviewCardContext);
	if (!context) throw Error("PreviewCard: missing context value");
	return context;
}
/** @internal */
function PreviewCard(props) {
	let $ = c(17), as, children, ref, restProps, selected;
	...
	let value = t0, t1;
	$[8] !== children || $[9] !== value ? (t1 = /* @__PURE__ */ jsx(PreviewCardContext.Provider, {
		value,
		children
	}), $[8] = children, $[9] = value, $[10] = t1) : t1 = $[10];
	let t2;
	return $[11] !== as || $[12] !== ref || $[13] !== restProps || $[14] !== selected || $[15] !== t1 ? (t2 = /* @__PURE__ */ jsx(StyledCard, {
		"data-ui": "PreviewCard",
		...restProps,
		forwardedAs: as,
		ref,
...
* @internal
*/
const ReferenceInputPreviewCard = styled(PreviewCard).withConfig({
	displayName: "ReferenceInputPreviewCard",
	componentId: "sc-mkoxdv-1"
})`min-height:36px;`, StyledButton = styled(Button$1).withConfig({
...
function SearchResultItem(t0) {
	...
	type?.name;
	let t2;
	$[13] === documentId ? t2 = $[14] : (t2 = getPublishedId$1(documentId), $[13] = documentId, $[14] = t2);
	let t3 = type?.name, t4;
	$[15] !== t2 || $[16] !== t3 ? (t4 = {
		id: t2,
		type: t3
	...
	$[25] !== documentId || $[26] !== state.disabledDocumentIds ? (t7 = state.disabledDocumentIds?.some((id) => id.includes(getPublishedId$1(documentId))), $[25] = documentId, $[26] = state.disabledDocumentIds, $[27] = t7) : t7 = $[27];
...
const GLOBAL_SEARCH_KEY_MODIFIER = typeof window < "u" && /Mac|iPod|iPhone|iPad/.test(window.navigator.platform) ? "Cmd" : "Ctrl", POPOVER_INPUT_PADDING = studioTheme.space[4], PREVIEW_FIELD_WEIGHT_MAP$2 = {
	title: 10,
	subtitle: 5,
	description: 1.5
}, CACHE$2 = /* @__PURE__ */ new WeakMap(), getTypeChain$2 = (type) => type ? [type, ...getTypeChain$2(type.type)] : [], isStringField$2 = (type) => type?.jsonType === "string", isPtField$2 = (type) => type?.jsonType === "array" && type.of.some((arrayMemberType) => getTypeChain$2(arrayMemberType).some(({ name }) => name === "block")), isSlugField$2 = (type) => getTypeChain$2(type).some(({ jsonType, name }) => jsonType === "object" && name === "slug");
...
const IMPLICIT_SCHEMA_TYPE_FIELDS = [
	"_id",
	"_type",
	"_createdAt",
	"_updatedAt",
...
const CACHE$1 = /* @__PURE__ */ new WeakMap(), PREVIEW_FIELD_WEIGHT_MAP$1 = {
	title: 10,
	subtitle: 5,
	description: 1.5
}, BASE_WEIGHTS$1 = {
	_id: { weight: 1 },
	_type: { weight: 1 }
}, ignoredBuiltInObjectTypes$1 = [
	"reference",
	"crossDatasetReference",
	"slug",
	"globalDocumentReference"
], getTypeChain$1 = (type) => type ? [type, ...getTypeChain$1(type.type)] : [], isPtField$1 = (type) => type?.jsonType === "array" && type.of.some((arrType) => getTypeChain$1(arrType).some(({ name }) => name === "block")), isStringField$1 = (schemaType) => schemaType ? schemaType?.jsonType === "string" : !1, isSlugField$1 = (schemaType) => getTypeChain$1(schemaType).some(({ jsonType, name }) => jsonType === "object" && name === "slug"), isSearchConfiguration$1 = (options) => isRecord(options) && "search" in options && isRecord(options.search);
...
* @internal
*/
const ORDERINGS_PROJECTION_KEY = "orderings";
/**
...
function createSearchQuery$1(searchTerms, searchParams, { perspective, variant, sort, isCrossDataset, tag, maxDepth, cursor, limit, params, comments, filter } = {}, { referenceIds, referenceWeight = 5 } = {}) {
	let hasReferenceIds = Array.isArray(referenceIds) && referenceIds.length > 0, flattenedSpecs = searchTerms.types.map((schemaType) => deriveSearchWeightsFromType2024({
		...
		cursor ?? []
	].flat(), orderingsExpressions = compiledSortEntries.map((entry) => entry.expression), projection = [
		"_type",
		"_id",
		"_originalId",
		`"${ORDERINGS_PROJECTION_KEY}": [${orderingsExpressions.join(", ")}]`
	].join(", "), query = [
		`*[${filters.join(" && ")}]`,
...
* @internal
*/
const createGroq2024Search = (typesFromFactory, client, factoryOptions) => function search(searchParams, searchOptions = {}) {
...
function partitionAndSanitizeSearchTerms(searchTerms) {
	let uniqueSearchTerms = uniq(searchTerms.map(toLower)), [searchPhrases, searchWords] = partition$1(uniqueSearchTerms, (term) => /^".*"$/.test(term));
	return {
...
}, ignoredBuiltInObjectTypes = [
	"reference",
	"crossDatasetReference",
	"slug"
], getTypeChain = (type) => type ? [type, ...getTypeChain(type.type)] : [], isPtField = (type) => type?.jsonType === "array" && type.of.some((arrType) => getTypeChain(arrType).some(({ name }) => name === "block")), isStringField = (schemaType) => schemaType ? schemaType?.jsonType === "string" : !1, isSlugField = (schemaType) => getTypeChain(schemaType).some(({ jsonType, name }) => jsonType === "object" && name === "slug"), isSearchConfiguration = (options) => isRecord(options) && "search" in options && isRecord(options.search);
...
const getUserSetWeight = (schemaType) => {
	...
	return typeof searchOptions?.search?.weight == "number" ? searchOptions.search.weight : null;
}, getHiddenWeight = (schemaType) => getTypeChain(schemaType).some((type) => type.hidden) ? 0 : null, getDefaultWeights = (schemaType) => typeof (getUserSetWeight(schemaType) ?? getHiddenWeight(schemaType)) == "number" ? null : 1, getPreviewWeights = (schemaType, maxDepth, isCrossDataset) => {
	let select = schemaType?.preview?.select;
	if (!select) return null;
...
const combinePaths = flow([
	flatten,
	union,
	compact
]), pathWithMapper = ({ mapWith, path }) => mapWith ? `${mapWith}(${path})` : path;
/**
...
function createSearchQuery(searchTerms, searchOpts = {}) {
	let { filter, params, tag, maxDepth, isCrossDataset, perspective, variant, sort, limit, comments } = searchOpts, specs = searchTerms.types.map((schemaType) => deriveSearchWeightsFromType({
		...
		...entry,
		projectionIndex: compiledSortEntries[index].projectionIndex
	})), orderingsExpressions = compiledSortEntries.map((entry) => entry.expression), orderingsProjection = `"${ORDERINGS_PROJECTION_KEY}": [${orderingsExpressions.join(", ")}]`, baseProjectionFields = [
		"_type",
		"_id",
		"_originalId"
...
const toGroqParams = (terms) => terms.reduce((acc, term, i) => (acc[`t${i}`] = `${term}*`, acc), {});
...
* @internal
*/
const createWeightedSearch = (types, client, factoryOptions) => function search(searchParams, searchOptions = {}) {
	...
	return versionedClient(client, apiVersion).observable.fetch(query, params, options).pipe(factoryOptions.unique ? map$1(removeDupes) : tap$1(), map$1((hits) => applyWeights(searchSpec, hits, terms)), searchOptions?.skipSortByScore ? tap$1() : map$1((hits) => sortBy(hits, (hit) => -hit.score)), map$1((hits) => ({
		type: "weighted",
		hits
	})));
}, searchStrategies$1 = {
...
}, createSearch = (searchableTypes, client, options) => {
...
function useSearchMaxFieldDepth(overrideClient) {
	let $ = c(4), isEnabled = useWorkspace().search?.unstable_partialIndexing?.enabled, workspaceClient = useClient(DEFAULT_STUDIO_CLIENT_OPTIONS), client = overrideClient || workspaceClient, t0;
	$[0] === client ? t0 = $[1] : (t0 = client.config(), $[0] = client, $[1] = t0);
	let dataset = t0.dataset;
	isEnabled || cachedSettings.set(dataset, of(INITIAL_LOADING_STATE.settings)), cachedSettings.has(dataset) || cachedSettings.set(dataset, fetchMaxDepth({ client }).pipe(shareReplay$1()));
	let t1;
	$[2] === dataset ? t1 = $[3] : (t1 = cachedSettings.get(dataset).pipe(map$1(_temp$9), startWith$1(INITIAL_LOADING_STATE), catchError$1(_temp2$3)), $[2] = dataset, $[3] = t1);
...
function useSearch(t0) {
	...
	let search = t1, t2;
	$[5] !== allowEmptyQueries || $[6] !== onComplete || $[7] !== onError || $[8] !== onStart || $[9] !== search ? (t2 = (inputValue$) => inputValue$.pipe(filter$1(nonNullable), map$1(sanitizeRequest), distinctUntilChanged$1(isEqual$1), debounce$1(_temp$8), tap$1(onStart), switchMap$1((request_0) => concat(of({
...
})`display:block;-webkit-font-smoothing:inherit;appearance:none;font:inherit;background:none;width:100%;text-align:left;border:0;margin:0;padding:0;outline:none;`, ToggleArrow = styled(ToggleArrowRightIcon).withConfig({
	displayName: "ToggleArrow",
	componentId: "sc-vw1iyn-1"
})`transform:${(props) => props.open ? "rotate(90deg)" : ""};`, Header = styled(Flex).withConfig({
	displayName: "Header",
	componentId: "sc-vw1iyn-2"
})`cursor:default;line-height:0;`, IconBox = styled(Flex).withConfig({
	displayName: "IconBox",
...
function Details(props) {
	let $ = c(30), children, icon, openProp, restProps, t0;
	...
	let title = t0 === void 0 ? "Details" : t0, [open, setOpen] = useState(openProp || !1), t1;
	$[6] === Symbol.for("react.memo_cache_sentinel") ? (t1 = () => setOpen(_temp$7), $[6] = t1) : t1 = $[6];
	let handleToggle = t1, t2, t3;
	$[7] === openProp ? (t2 = $[8], t3 = $[9]) : (t2 = () => setOpen(openProp || !1), t3 = [openProp], $[7] = openProp, $[8] = t2, $[9] = t3), useEffect(t2, t3);
	let t4 = open ? "" : void 0, t5;
	$[10] === open ? t5 = $[11] : (t5 = /* @__PURE__ */ jsx(Text, {
		size: 1,
		children: /* @__PURE__ */ jsx(ToggleArrow, { open })
	}), $[10] = open, $[11] = t5);
	let t6;
	$[12] !== t4 || $[13] !== t5 ? (t6 = /* @__PURE__ */ jsx(IconBox, {
		"data-open": t4,
		children: t5
	}), $[12] = t4, $[13] = t5, $[14] = t6) : t6 = $[14];
	let t7;
	$[15] === icon ? t7 = $[16] : (t7 = icon && /* @__PURE__ */ jsx(Box, {
		marginLeft: 1,
		children: icon
	}), $[15] = icon, $[16] = t7);
	let t8;
	$[17] === title ? t8 = $[18] : (t8 = /* @__PURE__ */ jsx(Box, {
		flex: 1,
		marginLeft: 1,
	...
	let t9;
	$[19] !== t6 || $[20] !== t7 || $[21] !== t8 ? (t9 = /* @__PURE__ */ jsx(HeaderButton, {
		type: "button",
		onClick: handleToggle,
		children: /* @__PURE__ */ jsx(Header, { children: /* @__PURE__ */ jsxs(Flex, {
			align: "center",
			children: [
				t6,
				t7,
	...
	let t10 = !open, t11;
	$[23] !== children || $[24] !== t10 ? (t11 = /* @__PURE__ */ jsx(Box, {
		hidden: t10,
		marginTop: 3,
		children
	}), $[23] = children, $[24] = t10, $[25] = t11) : t11 = $[25];
	let t12;
	return $[26] !== restProps || $[27] !== t11 || $[28] !== t9 ? (t12 = /* @__PURE__ */ jsxs(Box, {
		...restProps,
		children: [t9, t11]
	...
	return $[11] !== fieldDefinitions || $[12] !== filterDefinitions || $[13] !== operatorDefinitions || $[14] !== schema || $[15] !== setStoredSearch || $[16] !== storedSearch ? (t5 = {
		addSearch: (searchTerm, searchFilters) => {
			let validStoredFilters = (searchFilters || []).map(_temp$4).filter((filter_0) => validateFilter({
				...
				version: 3,
				recentSearches: [newSearchItem, ...storedSearch.recentSearches.filter((r) => !isEqual$1(omit(r, "created"), omit(newSearchItem, "created")))].slice(0, 5)
			};
...
function SortMenu() {
	...
	if ($[10] !== menuOrderings || $[11] !== ordering || $[12] !== t) {
		...
		bb0: {
			let currentMenuItem = menuOrderings.find((item) => isEqual$1(ordering, item) && !isSearchDivider(item));
			if (!currentMenuItem) {
				t12 = null;
				break bb0;
			}
...
function SearchProvider(t0) {
	...
	$[52] !== completeFilters.length || $[53] !== cursor || $[54] !== documentTypes || $[55] !== handleSearch || $[56] !== ordering || $[57] !== strategy || $[58] !== terms ? (t17 = () => {
		let orderingChanged = !isEqual$1(ordering, previousOrderingRef.current), cursorChanged = cursor !== previousCursorRef.current, termsChanged = !isEqual$1(terms, previousTermsRef.current);
...
export { isStaging as A, SearchResultItem as C, useSearchState as D, getSchemaField as E, ReferenceInputPreviewCard as F, usePreviewCard as I, useRenderingContext as L, hasSanityPackageInImportMap as M, StatusButton as N, defineSearchFilter as O, PreviewCard as P, useProjectOrganizationId as R, GLOBAL_SEARCH_KEY_MODIFIER as S, getPreviewValueWithFallback as T, hasSearchableTerms as _, SearchWrapper as a, createSearchQuery$1 as b, supportsTouch as c, Details as d, CustomTextInput as f, getSearchableTypes as g, useSearchMaxFieldDepth as h, RecentSearches as i, getSanityImportMapUrl as j, defineSearchFilterOperators as k, useStudioAppIdStore as l, useSearch as m, SearchPopover as n, SearchHeader as o, getDocumentTypesTruncated as p, SearchResults as r, Filters as s, SearchProvider as t, AppIdCacheProvider as u, createSearch as v, SearchResultItemPreview as w, compileFieldPath as x, getNextCursor as y };

//# sourceMappingURL=SearchProvider-DSLbCdgY.js.map