import { C as flattenNestedBlocks, S as PRESERVE_WHITESPACE_TAGS, _ as HTML_DECORATOR_TAGS, a as isElement, b as HTML_LIST_ITEM_TAGS, c as isNodeList, d as tagName, f as resolveJsType, g as HTML_BLOCK_TAGS, h as DEFAULT_SPAN, i as hasPreservedWhitespaceStyle, l as isPlaceholderAnnotation, m as DEFAULT_BLOCK, n as ensureRootIsBlocks, o as isMinimalBlock, p as BLOCK_DEFAULT_STYLE, r as hasMonospaceFontFamily, s as isMinimalSpan, t as defaultParseHtml, u as isPlaceholderDecorator, v as HTML_HEADER_TAGS, w as isEqualMarks, x as HTML_SPAN_TAGS, y as HTML_LIST_CONTAINER_TAGS } from "./helpers-CD8skykp.js";
...
function normalizeBlock(node, options = {}) {
	let schema = {
		block: { name: options.blockTypeName || "block" },
		span: { name: "span" },
		styles: [],
		lists: [],
		decorators: [],
	...
	let usedMarkDefs = [], allowedDecorators = options.allowedDecorators && Array.isArray(options.allowedDecorators) ? options.allowedDecorators : !1;
	return block.children = block.children.reduce((acc, child) => {
		let previousChild = acc[acc.length - 1];
		...
		return child._key = options.keyGenerator ? options.keyGenerator() : keyGenerator(), isSpan({ schema }, child) && (child.marks ? allowedDecorators && (child.marks = child.marks.filter((mark) => {
			let isAllowed = allowedDecorators.includes(mark), isUsed = block.markDefs?.some((def) => def._key === mark);
